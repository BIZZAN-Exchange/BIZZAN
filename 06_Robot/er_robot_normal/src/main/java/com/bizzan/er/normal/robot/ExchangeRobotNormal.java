...
public class ExchangeRobotNormal implements Runnable, ExchangeRobot{
	...
	public void processCurrentOrder() {
		...
		try {
			//盘口
			plateResult1 = HttpClientUtil.doHttpPost(this.plateURL, param, null);
			while (!(plateResult1 != null && !plateResult1.equals(""))){
				Thread.sleep(500);
				plateResult1 = HttpClientUtil.doHttpPost(this.plateURL, param, null);
			}
			if(plateResult1 != null && !plateResult1.equals("")) {
				JSONObject obj1 = JSONObject.parseObject(plateResult1);

				// 获取卖单委托（SELL）
				JSONArray askList = obj1.getJSONObject("ask").getJSONArray("items");

				// 获取买单委托（BUY）
				JSONArray bidList = obj1.getJSONObject("bid").getJSONArray("items");

				plateResult = HttpClientUtil.doHttpPost(this.currenOrderURL, param, null);
				while (!(plateResult != null && !plateResult.equals(""))){
					Thread.sleep(500);
					plateResult = HttpClientUtil.doHttpPost(this.currenOrderURL, param, null);
				}
	...
	protected void buildInitPlateOrder(int count, BigDecimal lastestPrice) {
		...
		// 以最新价为基准构建卖盘
		List<Double> temRands = getTemRand(count*2);
	...
	protected void buildSpecOrder(String direction, BigDecimal startPrice, BigDecimal endPrice) {
		...
		// 在两个价格中间追加3个订单
		List<Double> temRands = getTemRand(3);
		for(int i = 0; i < 3; i++) {
			double temRand = getAmount(i,temRands);
			BigDecimal amount = this.scaleAmount(temRand);
			// 以价差的25%、50%、75%进行递增
