  ...
  <div class="identbusiness" style=" padding: 81px;padding-top: 80px;">
    <div class="content">
      <div style="width: 80%;margin: 0 auto;margin-bottom: 60px;">
        ...
        <Steps
          class="apply-step"
          :current="certStatus == 2 ? 3 : certStatus == 3 ? 2 : certStatus"
        ...
        >
          ...
          <Step
            :title="certStatus == 1 || certStatus == 0 ? result : certStatus == 2 ? certified : shenheshibai"
          ></Step>
        </Steps>
        <!-- shangjiazhuxiao：商家注销  tijiaoshenqing：提交申请 shenheshibai：审核失败；passed：审核通过-->
        <Steps
          class="apply-step"
          :current="certStatus == 5 ? 1 : certStatus == 6 ? 2 : 3"
        ...
        <div v-if="certStatus == 6" style="width: 500px;margin: 0 auto;text-align: center;">
          <Button
            type="warning"
            style="width: 120px;background:#f0ac19;border-color:#f0ac19"
            @click="modal_return = true"
            long
            size="large"
            >{{ $t('uc.identity.shenagain') }}</Button
          >
        ...
        <div v-if="certStatus == 7" style="width: 500px;margin: 0 auto;text-align: center;">
          <Button
            ...
            long
            size="large"
            >{{ $t('uc.identity.sheqinggain') }}</Button
          >
        </div>
      ...
      <div class="ipshang" :class="certStatus != 0 ? 'applying' : ''">
        ...
        <Row style="margin-top:40px;">
          <Col span="8">
            <div class="business-function">
              ...
              <span
                style="font-size: 14px;overflow:hidden; overflow: hidden;text-overflow:ellipsis;display: block;white-space:nowrap;color:#999;"
                >{{ $t('uc.identity.zhusnhu') }}</span
              >
            </div>
          </Col>
          <Col span="8">
        ...
        <div v-show="certStatus === 0" style="text-align: center;font-size: 16px;margin-top:50px">
          ...
          <router-link
            target="_blank"
            to="/helpdetail?cate=1&id=11&cateTitle=常见问题"
            class="cur"
            style="color:#f0a70a"
            >{{ $t('uc.identity.agreement') }}</router-link
          >
        </div>
        <!-- 同意第一步的按钮 -->
        <div v-show="certStatus === 0" class="sq">
          <Button @click="apply" style="background:#f0a70a;color:#fff;outline:none;">{{
    ...
    <Modal v-model="modal_read">
      ...
      <div class="apply-note">
        ...
        <div style="text-align: left;padding: 30px 0;">
          ...
          <span>
            <font color="#f0a70a">{{ auditText }}</font
            >{{ $t('uc.identity.second.agreec') }}</span
          >
        </div>
        <Button @click="apply2" long style="font-size: 16px;background:#f0a70a;color:#fff;border:1px solid #f0a70a;">{{
      ...
      <div class="apply-content">
        ...
        <Form class="apply-form" :model="apply_form" label-position="top">
          ...
          <FormItem style="margin-top: 20px;">
            <Button
              style="width:100%;background:#f0a70a;color:#fff;border:1px solid #f0a70a;"
              type="info"
  ...
  methods: {
    ...
    apply2() {
      ...
      if (agreeFrozen == false) {
        this.$store.state.lang != 'English' && this.$Message.warning('请同意冻结相应数量的币')
        this.$store.state.lang == 'English' &&
          this.$Message.warning('Please agree to freeze the corresponding amount of currency')
        return
      }
    ...
    apply3(form) {
      if (this.apply_form.telno == '') {
        this.$store.state.lang != 'English' && this.$Message.error('请填写手机号')
        this.$store.state.lang == 'English' && this.$Message.error('Please fill in your cell phone number')
        return
      }
      if (this.apply_form.wechat == '') {
        this.$store.state.lang != 'English' && this.$Message.error('请填写微信号')
        this.$store.state.lang == 'English' && this.$Message.error('Please fill in your cell wechat number')
        return
      }
      if (this.apply_form.qq == '') {
        this.$store.state.lang != 'English' && this.$Message.error('请填写qq号')
        this.$store.state.lang == 'English' && this.$Message.error('Please fill in your cell qq number')
        return
      }
      if (this.apply_form.assetData == '') {
        this.$store.state.lang != 'English' && this.$Message.error('请上传资产证明')
        this.$store.state.lang == 'English' && this.$Message.error('Please upload the asset certificate')
        return
      }
      if (this.apply_form.tradeData == '') {
        this.$store.state.lang != 'English' && this.$Message.error('请上传交易证明')
        this.$store.state.lang == 'English' && this.$Message.error('Please upload the transaction certificate')
        return
      }
      var params = {}
